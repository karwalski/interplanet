# interplanet-time — Kotlin

Pure Kotlin port of the [planet-time.js](../../javascript/planet-time/planet-time.js) library (v1.1.0).
...
```
src/main/kotlin/interplanet/time/
├── Constants.kt     — Planet enum, J2000_MS, MARS_EPOCH_MS, LEAP_SECS, ORB_ELEMS
├── Orbital.kt       — helio pos, body distance, light travel, line of sight
├── TimeCalc.kt      — getPlanetTime, getMtc, getMarsTimeAtOffset
├── Models.kt        — PlanetTime, MtcResult, HelioPos data classes
├── Scheduling.kt    — findMeetingWindows
└── Formatting.kt    — formatLightTime
fixture-runner/
└── FixtureRunner.kt — reads reference.json, validates 54 cross-language entries
```
...
println("Mars time: ${pt.timeStr}")  // e.g. "14:23"
...
println("MTC Sol ${mtc.sol} — ${mtc.mtcStr}")
```
...
make test     # ./gradlew test (100+ assertions)
...
```sh
./gradlew run --args="../../c/fixtures/reference.json"
