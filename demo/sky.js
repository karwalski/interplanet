...
// ════════════════════════════════════════════════════════════════════════════
// HOLIDAY LOOKUP
// National holidays come from window.COUNTRY_HOLIDAYS (assets/holidays-data.js,
// generated by tools/generate-holidays.js using the date-holidays npm package).
// UN international days come from window.UN_DAYS (assets/un-days.js, also
// generated by tools/generate-holidays.js from the UN markdown file).
//
// _NAME_TO_ISO2 maps city.country strings to ISO alpha-2 codes used as keys
// in COUNTRY_HOLIDAYS. city.country is the full English name from the advanced
// city search (e.g. "United Kingdom"), but test helpers pass short forms ("UK").
// ════════════════════════════════════════════════════════════════════════════
const _NAME_TO_ISO2 = {
  // Full names (matches _COUNTRY_INDEX_INLINE)
  ...
  'Greece':'GR','Grenada':'GD','Guatemala':'GT','Guyana':'GY','Haiti':'HT',
  'Honduras':'HN','Hungary':'HU','Iceland':'IS','India':'IN','Indonesia':'ID',
  'Iran':'IR','Iraq':'IQ','Ireland':'IE','Israel':'IL','Italy':'IT',
  'Jamaica':'JM','Japan':'JP','Jordan':'JO','Kazakhstan':'KZ','Kenya':'KE',
  'Kiribati':'KI','Korea, South':'KR','Kuwait':'KW','Latvia':'LV','Lebanon':'LB',
  'Liberia':'LR','Libya':'LY','Liechtenstein':'LI','Lithuania':'LT','Luxembourg':'LU',
  'Macedonia':'MK','Malawi':'MW','Malaysia':'MY','Maldives':'MV','Mali':'ML',
  ...
  'Qatar':'QA','Romania':'RO','Russia':'RU','Senegal':'SN','Seychelles':'SC',
  ...
  'Tonga':'TO','Tunisia':'TN','Turkey':'TR','Tuvalu':'TV','Uganda':'UG',
  'Ukraine':'UA','United Kingdom':'GB','United States':'US','Uruguay':'UY',
  'Venezuela':'VE','Vietnam':'VN','Yemen':'YE','Zambia':'ZM','Zimbabwe':'ZW',
  'Gambia, The':'GM','Gambia':'GM','Samoa':'WS',
  // Extra territories
  'Singapore':'SG','Taiwan':'TW','Hong Kong':'HK','Macau':'MO',
  'North Korea':'KP','South Korea':'KR','Palestine':'PS',
  // Legacy short codes used by test helpers and demo cities
  'UK':'GB','USA':'US','SG':'SG','TW':'TW','HK':'HK',
};
...
/**
 * getTodayHoliday(country, tz, date)
 * Returns { national: string|null, international: string[] } for the given
 * city's date in its timezone.
 *
 * National holidays are keyed by full YYYY-MM-DD date in window.COUNTRY_HOLIDAYS
 * (generated by tools/generate-holidays.js via date-holidays), which handles
 * floating holidays (Easter, Thanksgiving, Eid, etc.) correctly.
 *
 * UN international days / weeks are from window.UN_DAYS (same generate script),
 * keyed by MM-DD and shown for every city regardless of country.
 *
 * Holiday names shown in: national holidays → English (from date-holidays);
 * UN days → current app language via t('holiday.un.*') keys in i18n.js,
 * falling back to the raw English name if no translation exists.
 */
function getTodayHoliday(country, tz, date) {
  try {
    ...
    let national = null;
    const iso2 = _NAME_TO_ISO2[country] || country; // handle raw ISO2 codes too
    const ch = (typeof window !== 'undefined' && window.COUNTRY_HOLIDAYS) || {};
    if (ch[iso2] && ch[iso2][fullDate]) {
      national = ch[iso2][fullDate];
    }

    // UN international days / weeks
    const ud = (typeof window !== 'undefined' && window.UN_DAYS) || {};
    const rawNames = ud[mmdd] || [];
    // Translate via i18n if available, otherwise use raw English name
    const international = rawNames.map(name => {
      const key = 'holiday.un.' + name.toLowerCase()
  ...
  } catch { return { national: null, international: [] }; }
}
...
function updateCityDisplay(city) {
  ...
  // Holiday badges — national (yellow) and UN international days (blue)
  const holidayEl = document.getElementById(`holiday-${city.id}`);
  if (holidayEl) {
    const { national, international } = getTodayHoliday(city.country, city.tz, now);
    let html = '';
    if (national) {
      html += `<span class="holiday-badge">${svgCalendar(10)} ${national}</span>`;
    }
    if (international.length) {
      html += international.map(n =>
        `<span class="holiday-badge-intl">🌍 ${n}</span>`
      ).join('');
    }
    if (html) { holidayEl.innerHTML = html; holidayEl.style.display = ''; }
  ...
  if (a11yEl) {
    ...
    const { national: todayNat, international: todayIntl } = getTodayHoliday(city.country, city.tz, now);
    if (todayNat) parts.push(`Public holiday: ${todayNat}.`);
    if (todayIntl.length) parts.push(`International observance: ${todayIntl.join(', ')}.`);
    const sr = getSunriseSunset(city.lat, city.lon, now);
