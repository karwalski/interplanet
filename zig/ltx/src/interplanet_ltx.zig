...
fn nodeToJson(allocator: Allocator, node: Node) ![]u8 {
    ...
    return std.fmt.allocPrint(allocator, "{{\"id\":\"{s}\",\"name\":\"{s}\",\"location\":\"{s}\",\"is_host\":{s}}}", .{ esc_id, esc_nm, esc_loc, if (node.is_host) "true" else "false" });
}
...
fn segmentToJson(allocator: Allocator, seg: Segment) ![]u8 {
    ...
    if (seg.speaker) |spk| {
        ...
        return std.fmt.allocPrint(allocator, "{{\"id\":\"{s}\",\"type\":\"{s}\",\"speaker\":\"{s}\",\"duration\":{d},\"start_offset\":{d}}}", .{ esc_id, esc_type, esc_spk, seg.duration, seg.start_offset });
    } else {
        return std.fmt.allocPrint(allocator, "{{\"id\":\"{s}\",\"type\":\"{s}\",\"speaker\":null,\"duration\":{d},\"start_offset\":{d}}}", .{ esc_id, esc_type, seg.duration, seg.start_offset });
    }
}
...
pub fn planToJson(allocator: Allocator, plan: Plan) ![]u8 {
    ...
    return std.fmt.allocPrint(allocator, "{{\"v\":\"{s}\",\"title\":\"{s}\",\"start\":\"{s}\",\"quantum\":{d},\"mode\":\"{s}\",\"nodes\":[{s}],\"segments\":[{s}]}}", .{ esc_v, esc_title, esc_start, plan.quantum, esc_mode, nodes_json, segs_json });
}
...
pub fn makePlanId(allocator: Allocator, plan: Plan) ![]u8 {
    ...
    for (plan.nodes) |n| {
        if (n.is_host) {
            host_id = n.id;
        } else if (std.mem.eql(u8, remote_id, "RX")) {
            remote_id = n.id;
        }
    }
    ...
    return std.fmt.allocPrint(allocator, "LTX-{s}-{s}-{s}-v2-{s}", .{ date_str, host_str, dest_str, hash8 });
}
...
pub fn buildNodeUrls(allocator: Allocator, plan: Plan, base_url: []const u8) ![]NodeUrl {
    ...
    for (plan.nodes, 0..) |node, i| {
        const session_url = try std.fmt.allocPrint(allocator, "{s}?node={s}{s}", .{ clean_base, node.id, hash });
        urls[i] = .{
            .node_id = node.id,
            .base_url = base_url,
...
fn addMinutesToIso(allocator: Allocator, iso: []const u8, minutes: u32) ![]u8 {
    ...
    return std.fmt.allocPrint(allocator, "{d:0>4}-{d:0>2}-{d:0>2}T{d:0>2}:{d:0>2}:00Z", .{ yr, mo, dy, new_hh, new_mm });
}
...
fn epochToIso(allocator: Allocator, epoch_s: i64) ![]u8 {
    ...
    const ss = @mod(tod, 60);
    return std.fmt.allocPrint(allocator, "{d:0>4}-{d:0>2}-{d:0>2}T{d:0>2}:{d:0>2}:{d:0>2}Z", .{ yr, m, d, hh, mm, ss });
}
