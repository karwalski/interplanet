/**
 * planet-time.js — Interplanetary Time Library  v1.0.0
 *
 ...
 * SATURN   Day ≈ 10.6h. Group 2.25 Saturn days → one period (~23.8 Earth h).
...
const PLANETS = {
  ...
  mercury: {
    ...
    epochMs: J2000_MS,
    notes: 'Solar day ≈ 176 Earth days. Shifts = 8 local hours each (~58 Earth days). 5 shifts on, 2 off.'
  },
  ...
  venus: {
    ...
    get localHourMs() { return this.solarDayMs / 24; },
    epochMs: J2000_MS,
    notes: 'Solar day ≈ 116.75 Earth days (retrograde). Rotation varies ~61 ppm (Margot 2021). 8-local-hour shifts.'
  },
  ...
  jupiter: {
    ...
    daysPerPeriod: 2.5,   // 2.5 J-days ≈ 24.8 Earth hours
    periodsPerWeek: 7, workPeriodsPerWeek: 5,
    workHoursStart: 8, workHoursEnd: 16, shiftHours: 8,
    get localHourMs() { return this.solarDayMs / 24; },
    epochMs: J2000_MS,
    notes: '1 Jupiter day ≈ 9.9h. Work periods = 2.5 Jupiter days (≈24.8h). 5 on, 2 off.'
  },

  saturn: {
    name: 'Saturn', symbol: '♄', color: '#e4d191',
    solarDayMs: 10.5606 * 3600000, // Mankovich et al. (2019, ApJ): 10h 33m 38s from ring seismology
    ...
    daysPerPeriod: 2.25,  // 2.25 S-days ≈ 23.8 Earth hours
    periodsPerWeek: 7, workPeriodsPerWeek: 5,
    workHoursStart: 8, workHoursEnd: 16, shiftHours: 8,
    get localHourMs() { return this.solarDayMs / 24; },
    epochMs: J2000_MS,
    notes: '1 Saturn day ≈ 10.5606h (Mankovich 2019 ring seismology). Work periods = 2.25 Saturn days (≈23.8h).'
  },
...
function getPlanetTime(planetKey, date = new Date(), tzOffsetHours = 0) {
  ...
  const periodInWeek = ((Math.floor(totalPeriods) % periodsPerWeek) + periodsPerWeek) % periodsPerWeek;
  const isWorkPeriod = periodInWeek < workPeriodsPerWeek;
  const isWorkHour   = isWorkPeriod && localHour >= p.workHoursStart && localHour < p.workHoursEnd;
...
const CONSTANTS = {
  ...
  SATURN_ROTATION_H: {
    value: 10.5606, unit: 'h',
    source: 'Mankovich et al. 2019 (ring seismology)', revision: '2019',
    uncertainty: '+0.031/−0.022 h',
